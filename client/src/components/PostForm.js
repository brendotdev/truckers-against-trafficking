...
const AddPostForm = ({
  ...
  const handleAddPost = async (values, { setSubmitting }) => {
    try {
      setSubmitting(true);
      const postId = await dispatch(createNewPost({...values, location}));
      setSubmitting(false);
  ...
  const handleUpdatePost = async (values, { setSubmitting }) => {
    try {
      setSubmitting(true);
      await dispatch(updatePost(postToEditId, { ...values, location }));
      setSubmitting(false);
          ...
          <Form className={classes.form}>
            ...
            <Checkbox
              ...
              onChange={() => {
                ...
                setFieldValue('isAnonymous', !isAnonymous); // Update Formik's state
              }}
              color="primary"
            />
