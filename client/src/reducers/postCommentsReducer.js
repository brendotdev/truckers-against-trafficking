...
export const createNewPost = (postObject) => {
  ...
  return async (dispatch) => {
    // const { location = {} } = postObject; // Extract location object
    const addedPost = await postService.addNew({
      ...postObject,
      isAnonymous: postObject.isAnonymous || false,
      lat: postObject?.location?.lat,  
      lng: postObject?.location?.lng,  
    });
