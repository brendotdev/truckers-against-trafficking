...
const { handleAnnonymousePosts } = require('../utils/handleAnnonymousPosts');
...
const getPosts = async (req, res) => {
  ...
  const paginatedPosts = {
    ...
    results: handleAnnonymousePosts(allPosts),
...
const getSubscribedPosts = async (req, res) => {
  ...
  const paginatedPosts = {
    previous: paginated.results.previous,
    results: handleAnnonymousePosts(subscribedPosts),
    next: paginated.results.next,
  };
...
const getSearchedPosts = async (req, res) => {
  ...
  if (longitude && latitude) {
    findQuery = {
      location: {
        $geoWithin: {
          $centerSphere: [[longitude, latitude], 2000000 / 6378137]
        }
      }
    };
  }
  ...
  const paginatedPosts = {
    previous: paginated.results.previous,
    results: handleAnnonymousePosts(searchedPosts),
    next: paginated.results.next,
  };
...
const flagPost = async (req, res) => {
  ...
  res.status(200).send({ message: 'Post flagged successfully.' });
}
